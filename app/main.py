...
def edit_product(
    ...
    with session_scope() as session:
        ...
        flash_banner = None
        if flash == "success":
            parts = [msg or f"✓ Generated {step} successfully."]
            if provider:
...
@app.post("/products/{product_id}/package")
def package_product(product_id: int):
    """Build a local upload-ready product package from generated artifacts."""
    with session_scope() as session:
        product = session.get(Product, product_id)
        if not product:
            raise HTTPException(status_code=404, detail="Product not found")
        ...
        artifacts = [_model_to_dict(artifact) for artifact in artifact_rows]

    artifact_map = _latest_artifacts_by_type(artifacts)
    for artifact_type in ("outline", "product_content", "listing_copy"):
        if artifact_type not in artifact_map:
            return _generation_redirect(
                product_id,
                "blocked",
                "package",
                msg=f"Missing required artifact: {artifact_type}",
            )

    result = build_product_package(product_data, artifact_map)
    ...
    with session_scope() as session:
        exists = session.scalar(
            select(ProductFile.id).where(
                ...
                ProductFile.file_path == zip_path,
            )
        )
        if not exists:
            session.add(
                ProductFile(
                    product_id=product_id,
                    file_label=f"Product package ZIP: {product_data['title']}",
                    file_path=zip_path,
                    file_type="package_zip",
                    notes="auto-generated by Sprint 1.8 package builder",
                )
            )
...
def build_product_package(product: dict, artifact_map: dict[str, dict]) -> dict:
    ...
    for artifact_type in ARTIFACT_TYPES:
        ...
        if not artifact:
            if artifact_type in ("qa_review", "distribution_post"):
                warnings.append(f"Optional artifact missing: {artifact_type}")
            continue
        path = source_dir / f"{artifact_type}.md"
    ...
    checklist_path = package_dir / "marketplace_checklist.md"
    _write_package_file(checklist_path, _build_marketplace_checklist(product, artifact_map), included_files)
    ...
    _write_package_file(manifest_path, json.dumps(manifest, indent=2), included_files)
...
def _latest_artifacts_by_type(artifacts: list[dict]) -> dict[str, dict]:
...
def _write_package_file(path: Path, content: str, included_files: list[Path]) -> None:
...
def _artifact_json(artifact: dict | None) -> dict:
    if not artifact:
        return {}
    try:
...
def _artifact_to_markdown(artifact: dict) -> str:
    artifact_type = artifact.get("artifact_type", "artifact")
    title = artifact.get("title") or artifact_type
    data = _artifact_json(artifact)
    if artifact_type == "outline" and data:
        sections = data.get("deliverable_sections") or []
        lines = [
            f"# {data.get('product_title') or title}",
            "",
            f"**Tagline:** {data.get('tagline', '')}",
            f"**Buyer persona:** {data.get('buyer_persona', '')}",
            f"**Core pain solved:** {data.get('core_pain_solved', '')}",
            "",
            "## Deliverable Sections",
            "",
        ]
        for section in sections:
            lines.append(f"- **{section.get('title', 'Section')}** — {section.get('purpose', '')} ({section.get('estimated_length', '')})")
        lines.extend([
            "",
            "## Marketplace Angles",
            "",
            f"- Etsy: {data.get('etsy_keyword_angle', '')}",
            f"- Gumroad: {data.get('gumroad_hook', '')}",
            f"- Risk notes: {data.get('risk_notes', 'none')}",
        ])
        return "\n".join(lines)
    if artifact_type == "product_content" and data:
        lines = [f"# {title}", ""]
        for section in data.get("sections") or []:
            lines.extend([f"## {section.get('title', 'Section')}", "", section.get("content", ""), ""])
        if data.get("total_word_count_estimate"):
            lines.append(f"_Estimated word count: {data['total_word_count_estimate']}_")
        return "\n".join(lines)
    if artifact_type == "listing_copy" and data:
        lines = [f"# {title}", ""]
        for platform in ("gumroad", "etsy", "sellfy", "payhip", "universal"):
            block = data.get(platform)
            if not isinstance(block, dict):
                continue
            lines.extend([f"## {platform.title().replace('_', ' ')}", ""])
...
def _is_printable_format(production_format: str | None) -> bool:
    text = (production_format or "").lower()
    return any(hint in text for hint in PRINTABLE_FORMAT_HINTS)
...
def _generate_printable_html_files(product: dict, content_data: dict, printable_dir: Path) -> list[Path]:
    ...
    for index, section in enumerate(sections, start=1):
        ...
        path = printable_dir / filename
        path.write_text(_render_printable_html(product, title, str(section.get("content") or "")), encoding="utf-8")
        written.append(path)
    return written


def _render_printable_html(product: dict, section_title: str, content: str) -> str:
    product_title = html_lib.escape(str(product.get("title") or "Printable"))
    section = html_lib.escape(section_title)
    paragraphs = [p.strip() for p in re.split(r"\n\s*\n", content) if p.strip()]
    body = "\n".join(f"<p>{html_lib.escape(paragraph)}</p>" for paragraph in paragraphs[:8])
    checklist = "\n".join(
        '<label class="check-row"><span class="box"></span><span class="line"></span></label>'
        for _ in range(5)
    )
    note_lines = "\n".join('<div class="write-line"></div>' for _ in range(6))
    return f"""<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <title>{section} - {product_title}</title>
  <style>
...
def _build_package_readme(product: dict, artifact_map: dict[str, dict]) -> str:
    listing = _artifact_json(artifact_map.get("listing_copy"))
    suggested_marketplaces = ["Gumroad", "Etsy", "Sellfy", "Payhip"]
    if listing.get("etsy"):
    ...
    return "\n".join([
        f"# {product.get('title')}",
        "",
        f"Production format: {product.get('production_format') or 'digital download'}",
        f"Suggested marketplace: {', '.join(dict.fromkeys(suggested_marketplaces))}",
        "",
        "## What Is Included",
        "",
        *included,
        ...
        "",
        "Download the ZIP, unzip it, then open or print the included files for personal use.",
        "",
        "## License Note",
        "",
        "Personal use only unless the operator changes this license before publishing.",
    ])


def _build_marketplace_checklist(product: dict, artifact_map: dict[str, dict]) -> str:
    listing = _artifact_json(artifact_map.get("listing_copy"))
    etsy = listing.get("etsy") or {}
    gumroad = listing.get("gumroad") or {}
    ...
    if isinstance(tags, list):
        ...
        tags_text = str(tags)
    return "\n".join([
        f"# Marketplace Upload Checklist — {product.get('title')}",
        "",
        "## Gumroad",
        "",
        "- Listing copy source: `source/listing_copy.md`",
        f"- Suggested title: {gumroad.get('title', product.get('title'))}",
        f"- Suggested price: ${product.get('price')}",
        "",
        "## Etsy",
        "",
        f"- Title: {etsy.get('title', '')}",
        f"- Tags: {tags_text}",
        f"- Category hint: {etsy.get('category_hint', '')}",
        "",
        "## Files To Upload",
        "",
        ...
        "",
        "- Image/mockup still required.",
        "- Confirm spelling, layout, and marketplace policy compliance.",
        "- Confirm no medical, legal, financial, political, IP, or trademark issues.",
