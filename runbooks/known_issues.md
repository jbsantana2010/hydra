...
- 2026-05-12 [sprint 2.8.1] **low** — Text overflow checks use estimated font widths rather than a real SVG renderer. Mitigation: estimates are conservative for bold/uppercase text and now catch buyer outcome/title overflow before launch readiness.
- 2026-05-12 [sprint 2.8.2] **low** — Composition scoring is heuristic and SVG-source based, not a human taste model or rendered screenshot QA. Mitigation: it now blocks obvious overcrowding signals and refined covers still require final operator review.
- 2026-05-12 [sprint 2.9] **low** — PNG export depends on CairoSVG being installed in the runtime. Mitigation: `cairosvg==2.7.1` is pinned in `app/requirements.txt` and installed locally for verification.
